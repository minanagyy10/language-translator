// ---- Language list (code, label) ----
  ...
  /*
    ---- Swap in Google Cloud Translation API instead (optional) ----
    Google's API requires an API key and billing enabled on a GCP project,
    and is meant to be called from a server (not directly from the browser)
    to avoid exposing your key. If you have a backend, replace the fetch
